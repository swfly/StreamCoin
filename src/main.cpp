...
uint256 hashGenesisBlock("0x3f084bc00a049f451826e7c1b5797533e51dabebcf3d93a49a08ae4f2663a11d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Streamcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Streamcoin Signed Message:\n";
...
{
    // Streamcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Streamcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1*60*60; // Streamcoin: 1 hour
static const int64 nTargetSpacing = 30; // Streamcoin: 15 secondes 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Streamcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Streamcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Streamcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3f084bc00a049f451826e7c1b5797533e51dabebcf3d93a49a08ae4f2663a11d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Basically meaningless";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1389361121;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 385753268;
        ...
        assert(block.hashMerkleRoot == uint256("0x42792e820b7e8de18aa684e9e82a686ad8bcfc51792d1b540796dfa34de4d0e8"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Streamcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StreamcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StreamcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StreamcoinMiner(CWallet *pwallet)
{
    printf("StreamcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("streamcoin-miner");
    ...
    try { loop {
        ...
        printf("Running StreamcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StreamcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StreamcoinMiner, pwallet));
}
